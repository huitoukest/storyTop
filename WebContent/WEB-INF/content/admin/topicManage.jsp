...
<Script language="JavaScript" type="text/javascript">
         ...
         {        	             
        	 topicManage_table_datagrid.datagrid({
         	    fit:true,
         	    url:'<%=request.getContextPath()%>/getAllTopics.json',
         	    border:false,
         	    pagination:true,
         	    idField:'id',
         	    ...
         	    columns:
					...
					{field:'type',title:'主题分类',width:60,formatter:
         	        ...
         	        {field:'user',title:'发布人',width:120,formatter:
           	        	function(value,row,index){
        				return value.name+" (编号:"+value.id+")";
        			}},        	        
         	        {field:'address',title:'交易地点',width:120,formatter:
           	        	function(value,row,index){
        				return value.name+" (编号:"+value.id+")";
        			}},
         	        {field:'thingSort',title:'交易物品类别',width:120,formatter:
           	        	function(value,row,index){
        				return value.name+" (编号:"+value.id+")";
        			}},
        ...
        function topic_topicManage_editTopic(){
         ...
         if(!wgUtils.isUsed(selections)||selections.length<=0)
            	...
            	alert("请只选择一行数据编辑!");
            		...
            		common_editTopic(data,"/admin/updateTopic.json",true,function(){topicManage_table_datagrid.datagrid('reload');
        ...
        //查看主题
        function topic_topicManage_lookTopic(){
        	
        	var selections=topicManage_table_datagrid.datagrid('getSelections');
            if(!wgUtils.isUsed(selections)||selections.length<=0)
               	{
               	 alert("您必须至少选择一个主题才能够查看!");
               	}
               else if(selections.length>1)
               	{
               	alert("请只选择一个主题!");
               		...
               		window.open("<%=request.getContextPath()%>/topic/showTopicDeteils.do?id="+id);
               	}
        }
        
        function topic_topicManage_addTopic()
        {
        	common_addTopic("/admin/addTopic.json",true,function(){topicManage_table_datagrid.datagrid('reload');},null);
        }
        ...
        function topic_topicManage_deleteTopic(r)
        {
        	if(r){
        		...
        		ids=ids.join(',');
        		var url="/admin/deleteTopics.json?ids="+ids;
...
<body>
<div id="topic_topicManage_mainDiv" class="easyui-layout" data-options="fit:true">
		<div id="topic_topicManage_searchContent" data-options="region:'north',title:'搜索条件:',split:true" style="height:230px;">
				...
				<form id="user_search_form" method="post" class="easyui-form">
				    ...
				    <div id="search_condition_div">
				         ...
				         <br/>
				         <table>
				         <tr><td><label for="user">发表人:</label>
				    <input class="easyui-validatebox" data-options="required:false" type="text" name="user"/></td><td><label for="search_type">交易类型:</label>
				...
				<a onclick="topic_topicManage_addTopic();" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" style="float:left;">增加</a>
				...
				<div class="datagrid-btn-separator"></div>
				<a onclick="" href="javascript:topic_topicManage_lookTopic();" class="easyui-linkbutton" data-options="iconCls:'icon-search',plain:true" style="float:left;">查看</a>
				<div class="datagrid-btn-separator"></div>
